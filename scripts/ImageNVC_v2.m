...
%      singleton*.
%
%      H = IMAGENVC retursn the handle to a new IMAGENVC or the handle to
...
% Last Modified by GUIDE v2.5 15-Jan-2025 12:38:18
%
% ========================================================================
...
%   - This flag should match the setting in Experiment_PB_DAQ.m
%
% IMPORTANT: If ImageFunctionPool.m contains microwave control code,
...
% in the ImageNVC_OpeningFcn function.
% ========================================================================
...
% ========================================================================
% INITIALIZE GLOBAL SEQUENCE STRUCTURE (gmSEQ)
% ========================================================================
% Initialize gmSEQ structure that ImageFunctionPool expects
% This is normally done by ExperimentFunctionPool('Initialize',...) but
% since we're running ImageNVC standalone, we need to initialize it here
% ========================================================================
global gmSEQ
% Check if gmSEQ is initialized as a struct with required fields
% After declaring 'global gmSEQ', if it didn't exist, MATLAB creates it as []
if isempty(gmSEQ) || ~isstruct(gmSEQ) || ~isfield(gmSEQ, 'meas')
    % Initialize gmSEQ with default values
    gmSEQ = struct();
    gmSEQ.bGo = 0;              % Flag to indicate if experiment is running
    gmSEQ.bGoAfterAvg = 0;      % Flag for continuing after averaging
    gmSEQ.m = 1;                % Default sequence index
    gmSEQ.meas = 'SPCM';        % Measurement type (default: Single Photon Counter)
    % Add other default fields as needed
else
    % Ensure all required fields exist (in case some are missing)
    if ~isfield(gmSEQ, 'bGo')
        gmSEQ.bGo = 0;
    end
    if ~isfield(gmSEQ, 'bGoAfterAvg')
        gmSEQ.bGoAfterAvg = 0;
    end
    if ~isfield(gmSEQ, 'm')
        gmSEQ.m = 1;
    end
    if ~isfield(gmSEQ, 'meas')
        gmSEQ.meas = 'SPCM';
    end
end
...
% ========================================================================
% ENSURE gmSEQ IS INITIALIZED BEFORE CALLING ImageFunctionPool
% ========================================================================
% ImageFunctionPool expects gmSEQ to be initialized. Ensure it's properly
% set up before calling RunCPS to avoid "Dot indexing" errors.
% ========================================================================
global gmSEQ
if isempty(gmSEQ) || ~isstruct(gmSEQ) || ~isfield(gmSEQ, 'meas')
    % Initialize gmSEQ with default values if not already initialized
    gmSEQ = struct();
    gmSEQ.bGo = 0;
    gmSEQ.bGoAfterAvg = 0;
    gmSEQ.m = 1;
    gmSEQ.meas = 'SPCM';  % This field is required by ImageFunctionPool>SetCounter
end
...
ImageFunctionPool('NewTrackFast',hObject, eventdata, handles);
numTrack = numTrack + 1;
...
% --- Executes on button press in ImageCorr.
function ImageCorr_Callback(hObject, eventdata, handles)
% hObject    handle to ImageCorr (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
ImageFunctionPool('TrackImageCorr',hObject, eventdata, handles);
disp(['ImageCorrelation running']);
pause(1);
...
% --- If Enable == 'on', executes on mouse press in 5 pixel border.
% --- Otherwise, executes on mouse press in 5 pixel border or over OffSetApply.
function OffSetApply_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to OffSetApply (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
