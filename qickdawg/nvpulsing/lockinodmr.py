...
class LockinODMR(NVAveragerProgram):
    ...
    cfg
        ...
        .odmr_lockin_swap_signal_reference (optional, default False)
            If True, swap which ADC shot is labeled signal vs reference after acquire (use if
            reference shows ODMR structure but signal is flat — shot order vs labels mismatch).
    ...
    def body(self):
        ...
        If cfg.odmr_reference_zero_gain_pulse is True, the reference window uses an explicit
        const pulse at cfg.odmr_reference_pulse_gain (default 1; gain 0 may fault on some
        firmware) with the same length as the signal window.
        '''
