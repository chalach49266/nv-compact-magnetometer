...
class NVAveragerProgram(QickRegisterManagerMixin, AcquireProgram):
    ...
    def acquire(self, load_pulses=True, readouts_per_experiment: Optional[int] = None,
        ...
        Parameters
        ----------
        readouts_per_experiment : int or None
            Number of readout *triggers* in the loop body (e.g. 2 for LockinODMR). Use ``None`` (default) to use the
            geometry from ``declare_readout`` (``ro_chs[*]['trigs']``). This is required for analog PL windows, where
            each trigger produces many decimated samples; forcing ``1`` undersizes the buffer and raises ValueError.
        load_pulses
        ...
        # Ensure reads_per_shot is set (robust to qick versions where setup_acquire runs differently)
        ...
        if readouts_per_experiment is None and hasattr(self, 'ro_chs') and self.ro_chs:
            self.reads_per_shot = [ro['trigs'] for ro in self.ro_chs.values()]
    ...
    def get_data_shape(self, readouts_per_experiment):
        ...
        if readouts_per_experiment is not None and readouts_per_experiment > 1:
    ...
    def acquire_decimated(self, readouts_per_experiment=None, raw_data=False, *arg, **kwarg):
        ...
        # qick versions differ: some AcquireMixin.acquire_decimated(soc, soft_avgs=...),
        # newer ones use program state only and reject soft_avgs.
        _parent_decimated = super().acquire_decimated
        if 'soft_avgs' in inspect.signature(_parent_decimated).parameters:
            data = _parent_decimated(qd.soc, soft_avgs=self.cfg['soft_avgs'], *arg, **kwarg)
        else:
            data = _parent_decimated(qd.soc, *arg, **kwarg)
    ...
    def check_cfg(self):
        ...
        mw_gain = getattr(self.cfg, 'mw_gain', None)
        if mw_gain is not None and mw_gain > 32767:
            raise ValueError("config.mw_gain should be between 0 and 32,767")
