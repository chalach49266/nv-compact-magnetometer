...
class NVConfiguration(ItemAttribute):
    ...
    def __init__(self):
        ...
        # LockinODMR / FixedFrequencyODMR: first ADC trigger is emitted with MW pulse(), second without.
        # If your **reference** (labeled MW off) shows ODMR dips and **signal** is flat, shot order vs labels
        # may be inverted for your stack — set True to swap signal/reference in analyze_data only.
        self.odmr_lockin_swap_signal_reference = False

        # LockinODMR / FixedFrequencyODMR: emit an explicit const pulse during the reference ADC
        # window at odmr_reference_pulse_gain (default 1). Some firmware errors on gain=0; use 1
        # as a minimal “off” drive. "No pulse" can still leave frequency-dependent LO/coupling.
        self.odmr_reference_zero_gain_pulse = False
        self.odmr_reference_pulse_gain = 1
    ...
    def _normalize_channel_value(self, name, value):
        """
        Normalizes readout channel aliases and validates against loaded firmware.
        ...
        Supported aliases:
          ...
          - ADC_B / B -> 2
        """
        if name not in ("adc_channel", "mw_readout_channel"):
            return value

        if isinstance(value, str):
            key = value.strip().upper()
            if key in self.ADC_CHANNEL_ALIASES:
                value = self.ADC_CHANNEL_ALIASES[key]
            else:
                try:
                    value = int(value)
                except ValueError as exc:
                    raise ValueError(
                        f"Invalid {name}={value!r}. Use an int channel index or one of "
                        f"{sorted(self.ADC_CHANNEL_ALIASES.keys())}."
                    ) from exc

        if not isinstance(value, int):
            raise TypeError(f"{name} must be an int or channel alias string, got {type(value).__name__}.")

        if value < 0:
            raise ValueError(f"{name} must be >= 0, got {value}.")

        try:
            n_readouts = len(self.soccfg["readouts"])
        except Exception:
            n_readouts = None

        if n_readouts is not None and value >= n_readouts:
            raise ValueError(
                f"{name}={value} is out of range for this firmware (available readout channels: 0..{n_readouts - 1}). "
                "If you need ADC_B as channel 2, load/build firmware exposing at least 3 readout channels."
            )
    ...
    def __setattr__(self, name, value):
        ...
        value = self._normalize_channel_value(name, value)
