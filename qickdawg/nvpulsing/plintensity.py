...
class PLIntensity(NVAveragerProgram):
    ...
    def acquire(self, verbose=False, *arg, **kwarg):
        '''
        ...
        ----------
        verbose : bool
            If True, print raw data (shape, min, max, mean) for debugging.
        counting_return : str
        ...
        float
            ...
            if not edge coutning
                returns average analog ADC level (sum / integration_time)
        '''
        ...
        if self.cfg.edge_counting:
            ...
            # Match LockinODMR, laser_on, etc.: divide by readout_integration_treg for ADC units.
            raw_sum = float(np.mean(data.astype('float')))
            val = raw_sum / self.cfg.readout_integration_treg
            if verbose:
                flat = np.asarray(data).flatten()
                print(f"  raw: shape={flat.shape}, min={flat.min()}, max={flat.max()}, mean={raw_sum:.1f} -> ADC units={val:.4f}")
            return val
