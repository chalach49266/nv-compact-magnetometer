...
class ReadoutWindow(NVAveragerProgram):
    ...
    cfg
        ...
        .adc_channel (required)
            int channel used for readout (0, 1, or 2)
            RFSoC4x2 mapping in qick-dawg: ADC_D=0, ADC_C=1, ADC_B=2

        .mw_channel (required)
            qick channel that provides microwave excitation
            0, 1, or 2 for RFSoC4x2 when firmware exposes ADC_B
    ...
    def initialize(self):
        ...
        # setup_readout() declares the ADC using readout_integration_treg. For time-domain
        # acquire_decimated(), that length must fit the RFSoC readout buffer (often 1024 samples).
        # This program's segment size is readout_length_treg — keep them aligned so copying
        # default_config (e.g. max PL integration 2**16-1) does not overflow the buffer.
        self.cfg.readout_integration_treg = self.cfg.readout_length_treg
